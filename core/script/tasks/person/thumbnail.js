...
TASKS.WikipediaPersonThumbnailTask = function(aEntityName,aEntityID,aEntityType,aOnSuccess, aOnError){
	var myself = this;
	this.eid = (new UTILS.Strings()).getEID(aEntityName,aEntityID);
	this.setMapping(TASKS.MAPPINGS.WIKIPEDIA_PERSON_THUMBNAIL_TASK, myself.eid);
	...
	this.run = function() {
		...
		var query = "SELECT ?thumbnail,?page, ?homepage " 
				+ "WHERE { "
	...
	{
		LOGGER.debug("WikipediaPersonThumbnailTask.valueReceived entityName:"+entityName+", entityID:"+entityID+", entityType:"+entityType+", data:"+data);
	...
	this.valueError = function(entityName,entityID,entityType, data) {
		LOGGER.debug("WikipediaPersonThumbnailTask.valueError entityName:"+entityName+", entityID:"+entityID+", entityType:"+entityType+", data:"+data);
		// show default picture in the navigation widget
		if (!myself.nextTask) 
			myself.onError(entityName,entityID,entityType, data);
		else {
			LOGGER.debug("WikipediaPersonThumbnailTask valueError : calling next Task:"+myself.nextTask.name);
			// call constructor to fallback to the next task;
			myself.nextTask.run(entityName,entityID,entityType, data);
		}
	};
};
TASKS.WikipediaPersonThumbnailTask.prototype = new TASKS.WikipediaTask();
TASKS.WikipediaPersonThumbnailTask.prototype.constructor = TASKS.WikipediaPersonThumbnailTask;

TASKS.FreebasePersonThumbnailTask = function(aEntityName,aEntityID,aEntityType,aOnSuccess, aOnError){
	LOGGER.debug("FreebasePersonThumbnailTask constructor : entityName:"+aEntityName+", entityID:"+aEntityID);
	var myself = this;
	this.eid = (new UTILS.Strings()).getEID(aEntityName,aEntityID);
	this.setMapping(TASKS.MAPPINGS.FREEBASE_PERSON_THUMBNAIL_TASK,myself.eid);
	...
	this.run = function() {
		LOGGER.debug("FreebasePersonThumbnailTask run : entityName:"+myself.entityName);
		myself.miner.getPersonThumbnailByName(myself.entityName,myself.valueReceived,  myself.valueError);
	};
	...
	this.valueError = function(data) {
		...
		// call constructor to fallback to the next task;
			...
			TASKS.FreebasePersonThumbnailTask.prototype.valueError.call(myself, myself.entityName,myself.entityID,myself.entityType, data);
		}
	};
};
TASKS.FreebasePersonThumbnailTask.prototype = new TASKS.FreebaseTask();
TASKS.FreebasePersonThumbnailTask.prototype.constructor = TASKS.FreebasePersonThumbnailTask;
