...
TASKS.WikipediaOrganisationThumbnailTask = function(aEntityName,aEntityID,aEntityType,aOnSuccess, aOnError){
	var myself = this;
	this.eid = (new UTILS.Strings()).getEID(aEntityName,aEntityID);
	this.setMapping(TASKS.MAPPINGS.WIKIPEDIA_ORGANISATION_THUMBNAIL_TASK, myself.eid);
	...
	this.run = function() {
		LOGGER.debug("WikipediaOrganisationThumbnailTask run : entityName:"+myself.entityName);
		
		var query = "SELECT ?thumbnail,?page,?homepage,?locationName " + "WHERE { "
				+ "	?person dbpedia-owl:thumbnail ?thumbnail ." + "	?person rdf:type dbpedia-owl:Organisation ."
				+ "	?person rdfs:label '" + (new UTILS.Strings()).cleanSparQlName(myself.entityName) + "'@en ."
				+ "	?person foaf:page ?page ." 
				+ "	OPTIONAL { ?person foaf:homepage ?homepage .} ." 
				+ "	OPTIONAL { ?person dbpedia-owl:location ?location . ?location rdfs:label ?locationName} ." 
				+ "  }";
	...
	this.valueError = function(entityName,entityID,entityType, data) {
		...
		if (!myself.nextTask) 
			...
			LOGGER.debug("WikipediaOrganisationThumbnailTask valueError : calling next Task:"+myself.nextTask.name);
			// call constructor to fallback to the next task;
			myself.nextTask.run(entityName,entityID,entityType, data);
		}
	};
};
TASKS.WikipediaOrganisationThumbnailTask.prototype = new TASKS.WikipediaTask();
TASKS.WikipediaOrganisationThumbnailTask.prototype.constructor = TASKS.WikipediaOrganisationThumbnailTask;

TASKS.FreebaseOrganisationThumbnailTask = function(aEntityName,aEntityID,aEntityType,aOnSuccess, aOnError){
	LOGGER.debug("FreebaseOrganisationThumbnailTask constructor : entityName:"+aEntityName+", entityID:"+aEntityID);
	var myself = this;
	this.eid = (new UTILS.Strings()).getEID(aEntityName,aEntityID);
	this.setMapping(TASKS.MAPPINGS.FREEBASE_ORGANISATION_THUMBNAIL_TASK,myself.eid);
	...
	this.run = function() {
		LOGGER.debug("FreebaseOrganisationThumbnailTask run : entityName:"+myself.entityName);
		myself.miner.getOrganisationThumbnailByName(myself.entityName,myself.valueReceived,  myself.valueError);
	};
	...
	this.valueError = function(data) {
		...
		// call constructor to fallback to the next task;
			...
			TASKS.FreebaseOrganisationThumbnailTask.prototype.valueError.call(myself, myself.entityName,myself.entityID,myself.entityType, data);
		}
	};
};
TASKS.FreebaseOrganisationThumbnailTask.prototype = new TASKS.FreebaseTask();
TASKS.FreebaseOrganisationThumbnailTask.prototype.constructor = TASKS.FreebaseOrganisationThumbnailTask;
