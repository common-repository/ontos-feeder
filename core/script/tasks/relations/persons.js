...
TASKS.PersonRelationsTask = function(eid, entityType, token){
	this.setMapping(TASKS.MAPPINGS.ONTOS_RELATIONS_2_PERSON_TASK);
	...
	this.setValue = function(data) {
		...
		$('#'+this.domElementName)[0].innerHTML ="<label onClick='window.navigationWidget.getPersons(\""+eid+"\",\""+entityType+"\", \""+token+"\")'>"+TASKS.TaskLabelCaptionProvider(this.getMapping())+"</label>";
	};
};
TASKS.PersonRelationsTask.prototype = new TASKS.OntosTask();
TASKS.PersonRelationsTask.prototype.constructor = TASKS.OntosTask;